//  FUNKCJA USUWAJĄCA WIERSZ Z TABELI PO KLIKNIĘCIU "USUŃ"
$(document).on("click", ".linkButtonKoszykUsun", function (e) {
    $(this).parents('tr').first().remove();
    usun();
});

//  FUNKCJA ZAPOBIEGAJĄCA WPISANIU ZNAKU ALFANUMERYCZNEGO
$(document).on("keydown", ".inputSzt", function (event) {
...
function dodajDoKoszyka(produktId, produktNazwa, produktCena) {
    ...
    if (!czyZawieraProdukt(id.value, nazwa.innerHTML, cena.innerHTML)) {    /*  jeśli funkcja nie znajdzie w tabeli produktu, 
        ...
        //row.addEventListener('click', function () { pobierzIdWiersza(this); }, false);
        ...
        element5.className += 'linkButtonKoszykAktualizuj';
        element5.addEventListener('click', function () { aktualizuj(); }, false);
        cell5.appendChild(element5);
        ...
        element6.className += 'linkButtonKoszykUsun';
...
function sumujProdukty() {          /*  podsumuj wartość koszyka po zmianie ceny   */
    ...
    for (var i = 0; i <= table.rows.length; i++) {
        kwota = (table.rows.item(i).cells.item(3).firstChild.innerHTML).replace(",", "."); /*  w wierszu 3. znajduje się pole "cena"   */
        ilosc = table.rows.item(i).cells.item(1).firstChild.value;                          /*  w wierszu 1. znajduje się pole "ilość"   */
        if (table.rows.item(i).cells.item(1).firstChild.value == "") {
            alert("Pole nie może być puste");
            return false;
        }
        sumaJS = sumaJS + (parseFloat(kwota) * parseInt(ilosc));
        sumaGotowa = sumaJS.toFixed(2);
        if (sumaGotowa == 0)
        {
            document.getElementById('LabelSuma').innerHTML = "";
        }
...
function usun() {
    ...
    var rowCount = table.rows.length;
    if (rowCount == 0) {
        document.getElementById("pustyKoszyk").innerHTML = "Twój koszyk jest pusty.";
    }
    sumujProdukty();
}

function pobierzIdWiersza(rowId) {
